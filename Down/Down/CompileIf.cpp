...
void CompileIf::compile(LinkedList& cTokenList, Token& begin, Token& end, LinkedActionList& listActionNodes, ActionNode& actionBefore)
{
	...
	if (current != nullptr)
	{
		...
		if (current->getEnum() == Token::ELSE) //Check if there is an else-statement 
		{
			int whileLevel = begin.getLevel();
			...
			for (TokenExpectation expectation : expected) //For each (TokenExpectation expectation in expected)
			{
				while (current->getEnum() == Token::NEWLINE)
				{
					current = current->next;
